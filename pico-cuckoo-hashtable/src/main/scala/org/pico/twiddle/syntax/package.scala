...
package object syntax {
  implicit class ByteOps_Ghg39HP(val self: Byte) extends AnyVal {
    ...
    def >>>>(offset: Long): Byte = if (offset > 0) (uint >>> offset.toInt).toByte else (uint <<  -offset.toInt).toByte
    def <<<<(offset: Long): Byte = if (offset > 0) (uint <<  offset.toInt).toByte else (uint >>> -offset.toInt).toByte
  ...
  implicit class ShortOps_Ghg39HP(val self: Short) extends AnyVal {
    ...
    def >>>>(offset: Long): Short = if (offset > 0) (uint >>> offset.toInt).toShort else (uint <<  -offset.toInt).toShort
    def <<<<(offset: Long): Short = if (offset > 0) (uint <<  offset.toInt).toShort else (uint >>> -offset.toInt).toShort
